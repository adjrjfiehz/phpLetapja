...
class InvoiceController extends Controller {
    ...
    public function invoice_create(){
        ...
        $Invoice->set_invoice_sub_total(trim($_POST['sub_total']));
        $Invoice->set_invoice_total_cgst(trim($_POST['total_cgst']));
        $Invoice->set_invoice_total_sgst(trim($_POST['total_sgst']));
        $Invoice->set_invoice_total_igst(trim($_POST['total_igst']));
        $Invoice->set_invoice_rs_in_word(trim($_POST['rs_in_word']));
        ...
        if(!empty($invoice_id)){
            ...
            if(isset($_POST['ii_json_data']) && !empty($_POST['ii_json_data']) && $_POST['ii_json_data']!='[]'){
                ...
                foreach ($ii_json_data_arr as $single) {
                    ...
                    $InvoiceItem->set_ii_amount($single['ii_amount']);
            ...
            $Invoice->set_invoice_total(trim($_POST['total']));
    ...
    public function invoice_delete(){
        ...
        $Invoice->set_invoice_id($_POST['invoice_id']);
        $view_data = $Invoice->delete_by_invoice_id();
        if(isset($view_data) && !empty($view_data)){
            $InvoiceItem->set_ii_invoice_id($_POST['invoice_id']);
            $invoice_item_data = $InvoiceItem->delete_item_by_invoice_id();
